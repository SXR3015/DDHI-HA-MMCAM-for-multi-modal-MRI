...
class _CAM:
    ...
    def _hook_a(self, module: nn.Module, input: Tensor, output: Tensor, idx: int = 0) -> None:
        ...
        if self._hooks_enabled:
            ...
            self.hook_a_list.append(output.data)
    ...
    def compute_cams(
        ...
        **kwargs: Any,
        ...
        weights, activations = self._get_weights(class_idx, scores, **kwargs)
        ...
        with torch.no_grad():
            ...
            for  activation in zip(activations):
                ...
                missing_dims = activation[0].ndim - weights.ndim  # type: ignore[union-attr]
                weight_tmp = weights[(...,) + (None,) * missing_dims]

                # Perform the weighted combination to get the CAM
                if activation[0].shape[1] != 512: # input  is not 512
                    cam = torch.nansum(weight_tmp[:,:activation[0].shape[1],...] * activation[0], dim=1) # dfc
                else:
                    cam = torch.nansum(weight_tmp[:, :, ...] * activation[0], dim=1)
                cam = F.interpolate(cam.unsqueeze(1), self._input[0][i].shape[2:], mode="trilinear",
                              align_corners=False)
                if self._relu:
